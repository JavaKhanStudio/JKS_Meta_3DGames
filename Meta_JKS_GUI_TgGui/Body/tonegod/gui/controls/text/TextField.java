...
public class TextField extends Element implements Control, KeyboardListener, TabFocusListener, MouseFocusListener, MouseButtonListener {
	...
	private String validatePureNumeric = "0123456789-";
	...
	public TextField(ElementManager screen, Vector2f position, Vector2f dimensions, Type type) {
				...
				if (isEnabled) {
					...
					if (valid) {
						switch(type)
						{
							...
							case PURE_NUMERIC :
							{
								if (validatePureNumeric.indexOf(nextChar) != -1) {
									textFieldText.add(caretIndex, nextChar);
									caretIndex++;
								}
								break  ;
